...
def evaluate(instance_data, n_instance, capacity, evaluate_func: callable):
    obj_1 = np.zeros(n_instance)
    obj_2 = np.zeros(n_instance)
    all_objs = []
    times = []
    ...
    for i, (coords, demand, distance_matrix) in enumerate(instance_data):
        ...
        archive = [
            (s, evaluate_solution(s, distance_matrix))
            for s in init_solutions
            if is_feasible_solution(s, demand, capacity)
        ]
        ...
        obj_1[i], obj_2[i] = mean_obj[0], mean_obj[1]
...
class BICVRPEvaluation(Evaluation):
    ...
    def evaluate_program(self, program_str: str, callable_func: callable):
        return evaluate(self._datasets, self.n_instance, self.cap, callable_func)
