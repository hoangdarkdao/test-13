...
def evaluate(instance_data, n_instance, problem_size, ref_point, capacity, eva: callable):
    ...
    for idx, (weight_lst, value1_lst, value2_lst) in enumerate(instance_data):
        ...
        Archive = [(s_, knapsack_value(s_, weight_lst, value1_lst, value2_lst, capacity))
                   for s_ in s if knapsack_value(s_, weight_lst, value1_lst, value2_lst, capacity)[0] > -1e5]
        ...
        obj_1[idx] = np.mean(objs[:, 0])  # total value1
        obj_2[idx] = np.mean(objs[:, 1])  # total value2
