...
def evaluate(instance_data, n_instance, problem_size, ref_point, eva: callable):
    ...
    Archives = []
    for idx, (instance, distance_matrix_1, distance_matrix_2) in enumerate(instance_data):
        ...
        # take the mean or best value of each objective per instance
        # (you can change np.mean to np.min if you want the best)
        obj_1[idx] = np.mean(objs[:, 0])
        obj_2[idx] = np.mean(objs[:, 1])
