...
class MOMCTS_AHD:
    def __init__(self,
        ...
        if 'bounds' not in kwargs:
            raise ValueError("Missing required argument 'bound' in kwargs.")
    ...
    def expand(self, mcts: MCTS, node_set: list[MCTSNode], cur_node: MCTSNode, option: str):
        ...
        if option == 's1':
            ...
            i = 0
            while i < 3:
                if self.review and self.long_term_guide is not None:
                    prompt = MOMCTSPrompt.get_prompt_s1(
                    self._task_description_str, path_set, self._function_to_evolve, self.long_term_guide)
                else:
                    prompt = MOMCTSPrompt.get_prompt_s1(
            ...
            while i < 3:
                ...
                if self.review and self.long_term_guide is not None:
                    prompt = MOMCTSPrompt.get_prompt_e2(
                                            self._task_description_str, indivs, self._function_to_evolve, self.long_term_guide)
                else:
                    prompt = MOMCTSPrompt.get_prompt_e2(
                    self._task_description_str, indivs, self._function_to_evolve)
            ...
            while i < 3:
                prompt = MOMCTSPrompt.get_prompt_m1(self._task_description_str, cur_node.individual,
                                                self._function_to_evolve, self.long_term_guide)
                func = self._sample_evaluate_register(prompt, option, func_only=True)
                if func is False:
                    is_valid_func = False
                    i += 1
            ...
            while i < 3:  # Retries giới hạn để flash
                ...
                prompt_generate = MOMCTSPrompt.get_flash_generate_code_prompt(
                    self._task_description_str, elites, self._function_to_evolve, self.long_term_guide
                )
    ...
    def run(self):
        ...
        self.mcts = MCTS('Root', num_objectives=2, alpha=self.alpha,
                         exploration_constant_0=self.lambda_0, bounds=self.bounds)
        ...
        while self._continue_loop():  # if current evaluation < max evaluation, still evaluate function
            ...
            current_hv = self._population.compute_hypervolume(self.bounds)
            print(f"Check current hv score: {current_hv}")
            prompt_phase1 = MOMCTSPrompt.get_flash_reflection_phase1_prompt(
                self._task_description_str, self._population, self._function_to_evolve
            )
            ...
            self.long_term_guide = self._sampler.get_thought(prompt_phase2) 
            ...
            new_hv = self._population.compute_hypervolume(self.bounds)
            print(f"Check new hv: {new_hv}")
            if new_hv - current_hv > 0: 
                print(f"HV improved from {current_hv:.6f} → {new_hv:.6f} — GOOD reflection.")
                self.good_reflections.append(self.long_term_guide)
            else:
                print(f"No HV improvement ({new_hv:.6f} ≤ {current_hv:.6f}) — BAD reflection.")
                self.bad_reflections.append(self.long_term_guide)

            save_json(self._tot_sample_nums, new_hv, self.long_term_guide, self.good_reflections, self.bad_reflections, save_dir="logs/reflection")
