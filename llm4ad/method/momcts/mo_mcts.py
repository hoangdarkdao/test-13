...
class MCTS:
    def __init__(self, root_answer: Any, num_objectives: int, bounds: List[list, list], exploration_constant_0: float = 0.1, alpha: float = 0.5):
    ...
    def _calculate_hypervolume(self, front: List[List[float, float]]) -> float: 
        '''
        Args:
            After normalization, all scores are in range [0, 1] + exploration term
            => [0, 1.5]
        '''
    ...
    def _calculate_multi_objective_ucb(self, child: MCTSNode, parent_visits: int) -> List[float]:
        ...
        for i in range(self.num_objectives):
            ...
            if child.visits > 0:
                normalized_vals = [
                    (r[i] - self.lower_bound[i]) / (self.upper_bound[i] - self.lower_bound[i])
                    for r in child.rewards_collected
                ]
                avg = np.mean(normalized_vals)
            else:
                avg = 0.0
