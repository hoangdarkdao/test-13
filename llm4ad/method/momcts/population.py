...
class Population:
    ...
    def survival(self, pop_size: int = None):
        ...
        for indi in unique_pop:
            obj1, obj2 = indi.score
            possitive_scores.append([obj1, obj2])
    ...
    def get_front(self) -> List[Function]:
       
        if not self._population:
            return []

        scores = np.array([ind.score for ind in self._population if ind.score is not None])
        if len(scores) == 0:
            return []
        ...
        return [self._population[i] for i in front_indices]

    def compute_hypervolume(self, bounds: list[list, list]) -> float:
         
        front = self.get_front()
        if not front:
            return 0.0

        scores = np.array([ind.score for ind in front if ind.score is not None])

        z_ideal = np.array(bounds[0]) 
        z_nadir = np.array(bounds[1])
